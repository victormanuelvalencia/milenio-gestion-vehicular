...
const crearViaje = async () => {
  ...
  creando.value = true
  try {
    await viajesService.crear(formularioCreacion.value)
...
const guardarEdicion = async (id) => {
  ...
  mensajeExito.value = ''
  try {
    await viajesService.actualizar(id, formularioEdicion.value)
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm text-center">
          <thead class="bg-slate-800 text-white text-xs uppercase tracking-wide">
            <tr>
              <th class="px-3 py-3 w-[10%]">Manifiesto</th>
              <th class="px-3 py-3 w-[10%]">Placa</th>
              <th class="px-3 py-3 w-[15%]">Conductor</th>
              <th class="px-3 py-3 w-[15%]">Ruta</th>
              <th class="px-3 py-3 w-[15%]">Empresa</th>
              <th class="px-3 py-3 w-[10%]">Flete</th>
              <th class="px-3 py-3 w-[10%]">Anticipo</th>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            >
              ...
              <td class="px-3 py-3 text-gray-600 truncate max-w-[120px]" :title="v.conductor?.nombre">{{ v.conductor?.nombre || '—' }}</td>
              <td class="px-3 py-3 text-gray-600">
                <div class="text-xs font-semibold text-gray-800">{{ v.origen }}</div>
                <div class="text-xs text-gray-500">a</div>
                <div class="text-xs font-semibold text-gray-800">{{ v.destino }}</div>
              </td>
              <td class="px-3 py-3 text-gray-600">{{ v.empresa }}</td>
              ...
              <td class="px-3 py-3">
                <div class="flex items-center justify-center gap-2">
                  <button @click="abrirModalGasto(v)" title="Agregar Gasto" class="flex items-center gap-1 text-xs font-medium text-emerald-600 hover:text-emerald-800 border border-emerald-200 hover:border-emerald-400 rounded-md px-2 py-1 transition-colors bg-emerald-50 hover:bg-emerald-100">
            ...
            >
              ...
              <td class="px-2 py-2">
                <select v-model="formularioEdicion.conductor_id" class="w-full px-1 py-1 text-xs border rounded">
                  <option v-for="c in conductores" :key="c.id" :value="c.id">{{ c.nombre }}</option>
                </select>
              </td>
              <td class="px-2 py-2">
                <input v-model="formularioEdicion.origen" placeholder="Origen" class="w-full px-1 py-1 mb-1 text-xs border rounded" />
                <input v-model="formularioEdicion.destino" placeholder="Destino" class="w-full px-1 py-1 text-xs border rounded" />
              </td>
              <td class="px-2 py-2"><input v-model="formularioEdicion.empresa" class="w-full px-1 py-1 text-xs border rounded" /></td>
