...
const filtros = ref({ proveedor_id: '' })
...
const buscar = async () => {
  ...
  try {
    ...
    const { data } = await reportesService.gastosPorProveedor(params)
...
const exportarExcel = () => exportarAExcel(resultados.value, columnasExportacion, 'Gastos_Proveedor')
const exportarPDF = () => exportarAPDF(resultados.value, columnasExportacion, 'Gastos_Proveedor', 'Reporte de Gastos por Proveedor')
</script>
...
<template>
  <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h2 class="text-2xl font-bold text-slate-800">Gastos por Proveedor</h2>
        <p class="text-sm text-slate-500 mt-1">Consulta todos los gastos realizados a nombre de un proveedor específico.</p>
      </div>
        ...
        <table class="w-full text-sm text-center table-fixed">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-if="resultados.length === 0">
              <td colspan="6" class="text-center py-10 text-gray-400">No hay resultados para el proveedor seleccionado.</td>
            </tr>
