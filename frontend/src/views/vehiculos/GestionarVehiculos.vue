...
const eliminarVehiculo = async (v) => {
  if (!confirm(`¿Estás seguro de eliminar el vehículo ${v.placa}? Esta acción no se puede deshacer.`)) return
  try {
    await vehiculosService.eliminar(v.id)
    mensajeExito.value = 'Vehículo eliminado correctamente.'
    await cargarVehiculos()
  } catch (e) {
    error.value = e.response?.data?.detail || 'Error al eliminar el vehículo. Es posible que tenga registros asociados.'
  }
}
...
<template>
  <div>
    ...
    <div v-else class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
      <table class="w-full text-sm text-center table-fixed">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <template v-for="v in vehiculosPaginados" :key="v.id">
            <tr v-if="editando !== v.id" class="hover:bg-slate-50 transition-colors">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center justify-center gap-3">
                  <template v-if="puedeEscribir">
                    ...
                    <button @click="eliminarVehiculo(v)" title="Eliminar" class="text-red-500 hover:text-red-700 transition-colors">
            ...
            <tr v-else-if="puedeEscribir && editando === v.id" class="bg-blue-50 border-l-4 border-blue-500">
    ...
    <!-- Modal de Creación (solo SUPERADMIN) -->
