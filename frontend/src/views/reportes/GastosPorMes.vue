...
const filtros = ref({ anio: new Date().getFullYear() })
const resultados = ref([])
const buscado = ref(false)
const cargando = ref(false)

const totalAnual = computed(() => resultados.value.reduce((s, r) => s + (r.total || 0), 0))
const formatValor = (v) => new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 }).format(v)

const buscar = async () => {
  cargando.value = true
  try {
    const { data } = await reportesService.gastosPorMes({ anio: filtros.value.anio })
...
const columnasExportacion = [
  { header: 'Mes', key: 'mes_nombre' },
  { header: 'Cantidad de Gastos', key: 'cantidad' },
  { header: 'Total Gastado', key: 'total' }
]

const exportarExcel = () => exportarAExcel(resultados.value, columnasExportacion, `Gastos_Mes_${filtros.value.anio}`)
const exportarPDF = () => exportarAPDF(resultados.value, columnasExportacion, `Gastos_Mes_${filtros.value.anio}`, `Reporte de Gastos por Mes - Año ${filtros.value.anio}`)
</script>
...
<template>
  <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h2 class="text-2xl font-bold text-slate-800">Gastos por Mes</h2>
        <p class="text-sm text-slate-500 mt-1">Visualiza el consolidado de gastos organizados mensualmente por año.</p>
      </div>
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100 mb-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        <div>
          ...
          <input v-model="filtros.anio" type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
        </div>
        <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-2xl font-bold text-slate-800">{{ formatValor(totalAnual) }}</p>
          </div>
        </div>
        <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p class="text-sm font-medium text-gray-500">Meses con Gastos</p>
        ...
        <table class="w-full text-sm text-center table-fixed">
          <thead class="bg-slate-800 text-white text-xs tracking-wide">
            <tr>
              ...
              <th class="px-4 py-3 w-[30%]">Total Gastado</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-if="resultados.length === 0">
              <td colspan="3" class="text-center py-10 text-gray-400">No hay gastos registrados para el año {{ filtros.anio }}.</td>
            </tr>
            <tr v-for="r in resultados" :key="r.mes" class="hover:bg-slate-50">
              <td class="px-4 py-3 font-medium text-slate-700">{{ r.mes_nombre }}</td>
