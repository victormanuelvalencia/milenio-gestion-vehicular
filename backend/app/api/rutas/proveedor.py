...
def obtener_proveedores(skip: int = 0, limit: int = 1000, bd: Session = Depends(obtener_bd)):
...
def crear_proveedor(proveedor: ProveedorCrear, bd: Session = Depends(obtener_bd), _: Usuario = Depends(requerir_superadmin)):
    return servicio_proveedor.crear(bd, proveedor_crear=proveedor)

@enrutador.put("/{id_proveedor}", response_model=ProveedorRespuesta)
def actualizar_proveedor(id_proveedor: int, proveedor: ProveedorActualizar, bd: Session = Depends(obtener_bd), _: Usuario = Depends(requerir_superadmin)):
...
def eliminar_proveedor(id_proveedor: int, bd: Session = Depends(obtener_bd), _: Usuario = Depends(requerir_superadmin)):
