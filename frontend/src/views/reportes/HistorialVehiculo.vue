...
const vehiculos = ref([])
const filtros = ref({ vehiculo_id: '' })
const resultados = ref([])
const buscado = ref(false)
const cargando = ref(false)

const totalHistorico = computed(() => resultados.value.reduce((s, g) => s + parseFloat(g.valor || 0), 0))
...
const buscar = async () => {
  ...
  try {
    ...
    const { data } = await reportesService.historialVehiculo(params)
...
const exportarExcel = () => exportarAExcel(resultados.value, columnasExportacion, 'Historial_Vehiculo')
const exportarPDF = () => exportarAPDF(resultados.value, columnasExportacion, 'Historial_Vehiculo', 'Historial de Vehículo')
</script>
...
<template>
  <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-6">
      <div>
        <h2 class="text-2xl font-bold text-slate-800">Historial de un Vehículo</h2>
        <p class="text-sm text-slate-500 mt-1">Revisa todos los gastos históricos registrados cronológicamente para un vehículo.</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-2xl font-bold text-slate-800">{{ formatValor(totalHistorico) }}</p>
          </div>
        </div>
        <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p class="text-sm font-medium text-gray-500">Registros Encontrados</p>
        ...
        <table class="w-full text-sm text-center table-fixed">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-if="resultados.length === 0">
              <td colspan="6" class="text-center py-10 text-gray-400">No hay resultados para el vehículo seleccionado.</td>
            </tr>
