...
def crear_usuario(usuario: UsuarioCrear, bd: Session = Depends(obtener_bd)):
    return servicio_usuario.crear(bd, usuario_crear=usuario)

@enrutador.put("/{id_usuario}", response_model=UsuarioRespuesta)
def actualizar_usuario(id_usuario: int, usuario: UsuarioActualizar, bd: Session = Depends(obtener_bd)):
    return servicio_usuario.actualizar(bd, id_usuario=id_usuario, usuario_actualizar=usuario)

@enrutador.delete("/{id_usuario}")
def eliminar_usuario(id_usuario: int, bd: Session = Depends(obtener_bd)):
...
def cambiar_estado(id_usuario: int, estado: CambiarEstado, bd: Session = Depends(obtener_bd)):
...
def cambiar_rol(id_usuario: int, rol: CambiarRol, bd: Session = Depends(obtener_bd)):
    return servicio_usuario.cambiar_rol(bd, id_usuario=id_usuario, nuevo_rol=rol.rol)

@enrutador.patch("/{id_usuario}/contrasena")
def cambiar_contrasena(id_usuario: int, contrasenas: CambiarContrasena, bd: Session = Depends(obtener_bd)):
