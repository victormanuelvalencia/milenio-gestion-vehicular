...
def obtener_gastos(skip: int = 0, limit: int = 10, bd: Session = Depends(obtener_bd)):
...
def crear_gasto(gasto: GastoCrear, bd: Session = Depends(obtener_bd)):
    return servicio_gasto.crear(bd, gasto_crear=gasto)

@enrutador.put("/{id_gasto}", response_model=GastoRespuesta)
def actualizar_gasto(id_gasto: int, gasto: GastoActualizar, bd: Session = Depends(obtener_bd)):
    return servicio_gasto.actualizar(bd, id_gasto=id_gasto, gasto_actualizar=gasto)

@enrutador.delete("/{id_gasto}")
def eliminar_gasto(id_gasto: int, bd: Session = Depends(obtener_bd)):
