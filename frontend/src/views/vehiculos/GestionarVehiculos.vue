...
const crearVehiculo = async () => {
  ...
  try {
    ...
    errorCrear.value = e.response?.data?.detail || 'Error al crear el vehículo. Verifica los datos.'
  } finally {
    creando.value = false
  }
}
    ...
    <div v-if="mostrarModalCrear" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-full">
        ...
        <div class="p-6 overflow-y-auto">
          ...
          <form @submit.prevent="crearVehiculo" class="space-y-4">
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Placa *</label>
              <input v-model="formularioCreacion.placa" required placeholder="ej. ABC123" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Marca *</label>
              <input v-model="formularioCreacion.marca" required placeholder="ej. Chevrolet" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Modelo *</label>
              <input v-model="formularioCreacion.modelo" required placeholder="ej. FRR" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Año *</label>
              <input v-model.number="formularioCreacion.ano" type="number" required min="1900" :max="new Date().getFullYear() + 1" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Estado inicial</label>
              <div class="flex gap-4">
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" :value="true" v-model="formularioCreacion.estado" class="text-blue-600" />
                  <span class="text-sm text-gray-700">Activo</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" :value="false" v-model="formularioCreacion.estado" class="text-blue-600" />
            ...
            <div class="flex gap-3 pt-4 border-t border-gray-100 mt-6">
              ...
              <button type="submit" :disabled="creando" class="flex-1 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold rounded-lg transition-colors text-sm shadow-sm">
                {{ creando ? 'Guardando...' : 'Crear Vehículo' }}
              </button>
            </div>
          </form>
        </div>
