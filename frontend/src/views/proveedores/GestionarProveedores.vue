...
const cargarProveedores = async () => {
  ...
  } catch { error.value = 'Error al cargar los proveedores.' }
  finally { cargando.value = false }
}

const abrirModalCrear = () => { errorCrear.value = ''; formularioCreacion.value = { nombre: '', nit: '' }; mostrarModalCrear.value = true }
...
const crearProveedor = async () => {
  ...
  try {
    ...
    cerrarModalCrear(); await cargarProveedores()
  } catch (e) { errorCrear.value = e.response?.data?.detail || 'Error al crear el proveedor.' }
  finally { creando.value = false }
}

const iniciarEdicion = (p) => { editando.value = p.id; formularioEdicion.value = { ...p } }
...
const guardarEdicion = async (id) => {
  ...
  try {
    ...
    editando.value = null; await cargarProveedores()
  } catch (e) { error.value = e.response?.data?.detail || 'Error al actualizar el proveedor.' }
}

const eliminarProveedor = async (p) => {
  if (!confirm(`¿Estás seguro de eliminar el proveedor "${p.nombre}"?`)) return
  error.value = ''; mensajeExito.value = ''
  try {
    await proveedoresService.eliminar(p.id)
    mensajeExito.value = 'Proveedor eliminado correctamente.'
    await cargarProveedores()
  } catch (e) { error.value = e.response?.data?.detail || 'Error al eliminar el proveedor.' }
}
...
const proveedoresPaginados = computed(() => {
  ...
  return proveedoresFiltrados.value.slice(inicio, inicio + POR_PAGINA)
})
...
<template>
  <div>
    ...
    <div class="mb-4 relative">
      ...
      <input v-model="busqueda" @input="resetPagina" type="text" placeholder="Buscar por nombre o NIT..." class="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm" />
    </div>
    ...
    <div v-if="error" class="mb-4 p-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm flex justify-between">{{ error }}<button @click="error = ''" class="font-bold">x</button></div>
      ...
      <table class="w-full text-sm text-center table-fixed">
        <thead class="bg-slate-800 text-white text-xs tracking-wide">
          <tr>
            ...
            <th class="px-4 py-3 w-[30%]">NIT</th>
            <th class="px-4 py-3 w-[35%]">Acciones</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
          <tr v-if="proveedoresFiltrados.length === 0">
            <td colspan="3" class="text-center py-10 text-gray-400">{{ busqueda ? 'Sin resultados para la búsqueda.' : 'No hay proveedores registrados.' }}</td>
          </tr>
          <template v-for="p in proveedoresPaginados" :key="p.id">
            <tr v-if="editando !== p.id" class="hover:bg-slate-50">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center justify-center gap-3">
                  <template v-if="puedeEscribir">
                    ...
                    <button @click="eliminarProveedor(p)" title="Eliminar" class="text-red-500 hover:text-red-700 transition-colors">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                    </button>
                  </template>
                  <span v-else class="text-xs text-gray-400 italic">Solo lectura</span>
                </div>
              </td>
            </tr>
            <tr v-else-if="puedeEscribir && editando === p.id" class="bg-blue-50 border-l-4 border-blue-500">
    ...
    <div v-if="totalPaginas > 1" class="flex items-center justify-center gap-1 mt-4">
      ...
      <span class="ml-2 text-sm text-gray-500">Página {{ paginaActual }} de {{ totalPaginas }} · {{ proveedoresFiltrados.length }} registros</span>
    </div>
    ...
    <div v-if="mostrarModalCrear && puedeEscribir" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <form @submit.prevent="crearProveedor" class="p-6 space-y-4">
          <div v-if="errorCrear" class="p-3 bg-red-50 border border-red-200 text-red-700 rounded-lg text-sm">{{ errorCrear }}</div>
          <div><label class="block text-sm font-medium text-gray-700 mb-1">Nombre *</label><input v-model="formularioCreacion.nombre" required placeholder="ej. Estación El Progreso" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" /></div>
          <div><label class="block text-sm font-medium text-gray-700 mb-1">NIT *</label><input v-model="formularioCreacion.nit" required placeholder="ej. 900123456-1" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" /></div>
          <div class="pt-4 flex justify-end gap-3 border-t border-gray-100 mt-6">
            ...
            <button type="submit" :disabled="creando" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-lg hover:bg-blue-700 disabled:opacity-50">{{ creando ? 'Guardando...' : 'Crear Proveedor' }}</button>
          </div>
        </form>
      </div>
    </div>
