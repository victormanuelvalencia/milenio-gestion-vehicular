...
def crear_tipo_gasto(tipo: TipoGastoCrear, bd: Session = Depends(obtener_bd)):
    return servicio_tipo_gasto.crear(bd, tipo_crear=tipo)

@enrutador.put("/{id_tipo}", response_model=TipoGastoRespuesta)
def actualizar_tipo_gasto(id_tipo: int, tipo: TipoGastoActualizar, bd: Session = Depends(obtener_bd)):
    return servicio_tipo_gasto.actualizar(bd, id_tipo=id_tipo, tipo_actualizar=tipo)

@enrutador.delete("/{id_tipo}")
def eliminar_tipo_gasto(id_tipo: int, bd: Session = Depends(obtener_bd)):
